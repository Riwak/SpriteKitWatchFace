...
- (instancetype)initWithCoder:(NSCoder *)coder
{
	...
	if (self) {
		...
		self.colorRegionStyle = ColorRegionStyleHalf;
		self.showDate = YES;
        self.showBattery = YES;
        self.showDailyMessage = YES;
        self.showWeather = YES;
        self.batteryCenter = YES; // NO
        self.romanNumerals = NO; // YES
        self.romanBattery = NO; // NO
...
-(void)setupTickmarksForRoundFaceWithLayerName:(NSString *)layerName
{
    ...
    if (self.showWeather)
    {
        ...
        SKLabelNode *numberLabel2 = [SKLabelNode labelNodeWithAttributedText:labelText2];
        
        numberLabel2.position = CGPointMake(0+numeralDelta, 30);
        
        [faceMarkings addChild:numberLabel2];
    }
...
/*    if (self.showDailyMessage)
    {
        ...
        NSAttributedString *labelText = [[NSAttributedString alloc] initWithString:[[NSString stringWithFormat:@"TESTTEST"] uppercaseString] attributes:attribs];
...
-(void)updateHands
{
	...
	else if (self.colorRegionStyle == ColorRegionStyleHalf)
	{
		colorRegion.alpha = 1.0;
        // Section Diagonale
		colorRegion.zRotation =  M_PI_2 -(2*M_PI)/60.0 * (CGFloat)(21 + 1.0/60.0*30);
		colorRegionReflection.zRotation =  M_PI_2 - (2*M_PI)/60.0 * (CGFloat)(21 + 1.0/60.0*30);
